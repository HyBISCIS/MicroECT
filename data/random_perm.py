...
try: 
    from plot import draw_line, draw_image
except Exception as e: 
    from .plot import draw_line, draw_image


"""Perlin noise implementation.
Sourc: https://gist.github.com/eevee/26f547457522755cb1fb8739d0ea89a1
"""
# Licensed under ISC
def smoothstep(t):
...
def generate_noise(width, height, max_y, seed): 
    
    noise = PerlinNoiseFactory(dimension=1, octaves=4,  tile=(0, 100), unbias=True)

    thickness_profile = []
    x_list = []
    
    random_image = np.ones((int(height), int(width)))

    for t in np.arange(0, 0.01*width, 0.01):
        n = noise(t)

        x = int(np.round(100*t)) 
        y =  interp1d([0, 1], [0, height / 2])(np.abs(n))

        x_list.append(x)
        thickness_profile.append(y)
        
        random_image[0:int(y), x] = 0.25 
        
    return random_image, thickness_profile



def set_perm(perm, tri, pts, random_image):    
    tri_centers = np.mean(pts[tri], axis=1)  

    tri_centers[:,0] = np.round(tri_centers[:,0]*1e6)
    tri_centers[:,1] = np.round(tri_centers[:,1]*1e6)

    tri_centers[:,0] = tri_centers[:,0] - np.min(tri_centers[:,0])
    tri_centers[:,1] = tri_centers[:,1] - np.min(tri_centers[:,1]) 
   
    for i in range(len(perm)):
        perm[i] = random_image[int(tri_centers[i, 1]), int(tri_centers[i, 0])]
    
    return perm 


def main():
    random_image, thickness_profile = generate_noise(width=200, height=100, max_y=30, seed=0) 

    draw_line(np.arange(0, len(thickness_profile)), thickness_profile, "thickness profile", "" , "", "thc.png")
    draw_image(random_image, "Ranom Image", "", "", "random.png")
